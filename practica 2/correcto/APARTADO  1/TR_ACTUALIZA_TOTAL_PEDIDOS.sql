...
BEGIN
  ...
  ELSIF DELETING THEN
    ...
    UPDATE pedidos
        SET importe_total = importe_total - (:old.precio_con_comision * :old.unidades)
        WHERE pedidos.codigo = :old.pedido;
  ELSIF UPDATING THEN
   DBMS_output.put_line('update');
  UPDATE pedidos
      SET importe_total = importe_total - (:old.precio_con_comision * :old.unidades) + (:new.precio_con_comision * :new.unidades)
      WHERE pedidos.codigo = :new.pedido;
